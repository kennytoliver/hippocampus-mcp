...
:root[data-theme="light"] .nav.on{color:#fff;
  background:linear-gradient(135deg,var(--acc),#1f5fd6);
  box-shadow:0 4px 14px rgba(30,107,255,.30)}
:root[data-theme="light"] body{
  background:
    radial-gradient(900px 520px at 82% -10%,rgba(34,211,238,.14),transparent 60%),
    radial-gradient(800px 480px at -6% 104%,rgba(30,107,255,.10),transparent 62%),
    var(--d1)}
:root[data-theme="light"] .side{background:var(--chrome)}
:root[data-theme="light"] .side::after{opacity:.35}
:root[data-theme="light"] .toolbar{background:var(--chrome)}
:root[data-theme="light"] .btn{background:rgba(30,107,255,.08);color:#1348b8}
:root[data-theme="light"] .nav.on{color:#fff}
:root[data-theme="light"] .toast-item{box-shadow:0 10px 30px rgba(15,23,42,.16)}
:root[data-theme="light"] .split-side{box-shadow:0 1px 3px rgba(15,23,42,.06)}
:root[data-theme="light"] .mini{background:#fff}

*{margin:0;padding:0;box-sizing:border-box}
html,body{height:100%}
body{background:var(--d1);color:var(--ink);font-family:var(--sans);
...
.ngroup{font-size:11px;color:var(--faint);padding:8px 10px 6px;letter-spacing:.04em}
.nav{display:block;padding:7px 10px;border-radius:8px;font-size:13.5px;
  color:var(--sub);cursor:pointer;text-decoration:none;margin-bottom:2px;
  transition:background .12s}
.nav:hover{background:var(--d2);color:var(--ink)}
.nav.on{background:var(--d3);color:var(--ink)}
...
.toolbar input{flex:1;max-width:680px;background:var(--d3);border:1px solid var(--line);
...
/* ── 标题与文字 ──────────────────── */
h2{font-family:var(--serif);font-size:24px;font-weight:700;margin-bottom:4px}
...
.pri{font-size:11px;color:var(--warn)}
.score{font-size:11px;color:var(--acc)}
.mem .content{font-size:14px;line-height:1.75}
.mem.indb{opacity:.42}
.ck{width:14px;height:14px;accent-color:var(--acc);cursor:pointer;flex-shrink:0}
...
.agent{background:var(--d2);border:1px solid var(--line);border-radius:12px;
  padding:16px 18px}
...
.mini{border:1px solid var(--line);background:var(--d3);color:var(--ink);
  border-radius:7px;padding:5px 12px;font-size:12px;cursor:pointer;font-family:var(--sans)}
.mini:hover{background:#434345}
.mini.warn{color:var(--bad)}
.msg{background:var(--d2);border:1px solid var(--line);border-radius:10px;
  padding:12px 16px;font-size:12.5px;line-height:1.75;margin-bottom:14px;white-space:pre-wrap}
.msg.ok{border-color:rgba(48,209,88,.45)}
.msg.err{border-color:rgba(255,69,58,.5)}
.textin{flex:1;min-width:240px;background:var(--d1);border:1px solid var(--line);
  border-radius:8px;padding:8px 13px;font-size:13px;color:var(--ink);outline:none;
...
textarea{width:100%;background:var(--d1);border:1px solid var(--line);
  border-radius:8px;color:var(--ink);padding:11px 13px;font-size:13.5px;
...
select,.formrow input[type=text]{background:var(--d1);border:1px solid var(--line);
...
/* ── 交接卡输出 ──────────────────── */
.handoff-out{background:var(--d1);border:1px solid var(--line);color:#c7c7cc;
  border-radius:12px;padding:18px 20px;font-size:12.5px;line-height:1.8;
  white-space:pre-wrap;display:none;margin-top:14px;font-family:var(--sans)}
.empty{text-align:center;color:var(--faint);font-size:12px;padding:44px 0}
.listhead{display:flex;justify-content:space-between;align-items:center;
  margin:22px 0 12px}
.listhead .t{font-family:var(--serif);font-size:17px;font-weight:700}
...
.nav::before{
  ...
  background:var(--acc);transform:translateY(-50%) scaleY(0);
  transition:height .22s var(--ease),transform .22s var(--ease);
}
.nav.on::before{height:18px;transform:translateY(-50%) scaleY(1)}
.nav.on{box-shadow:inset 0 0 0 1px var(--line)}

/* 统计卡：数字更大、层级更清晰 */
.stat .v{font-size:30px;letter-spacing:-.5px;font-variant-numeric:tabular-nums}
.stat .k{font-size:11.5px;letter-spacing:.4px;text-transform:uppercase;opacity:.72}
.stat{position:relative;overflow:hidden}
.stat::after{
  content:"";position:absolute;right:-24px;top:-24px;width:76px;height:76px;border-radius:50%;
  background:radial-gradient(circle at 30% 30%,rgba(10,132,255,.22),transparent 70%);
}

/* 标题层级拉大 */
h2{font-size:21px;letter-spacing:-.3px}
.sbrand h1{font-size:19px;letter-spacing:.2px}

/* 搜索框聚焦发光（有细节但不刺眼） */
.toolbar input:focus,.textin:focus{box-shadow:0 0 0 3px rgba(10,132,255,.16)}
...
.toast-item{
  background:var(--toastbg);border:1px solid var(--line2);border-radius:10px;
...
/* ───────── 强调色体系（琥珀金主色 + 青绿辅色）───────── */
body{background:
  radial-gradient(900px 520px at 82% -10%,rgba(34,211,238,.10),transparent 60%),
  radial-gradient(800px 480px at -6% 104%,rgba(46,125,255,.13),transparent 62%),
  var(--d1)}
.side{background:rgba(26,26,29,.86);border-right-color:var(--line)}
.side::after{content:"";position:absolute;left:0;top:0;bottom:0;width:1px;
  background:linear-gradient(180deg,transparent,rgba(46,125,255,.65),transparent)}
.sbrand h1{color:var(--ink)}
.sbrand p{color:var(--acc);opacity:.9;letter-spacing:.3px}

/* 导航：激活项金色，指示条更醒目 */
/* 导航：胶囊激活态（更大胆） */
.nav{border-radius:10px;padding-left:14px}
.nav.on{color:#fff;background:linear-gradient(135deg,var(--acc),#1f5fd6);
  box-shadow:0 6px 18px rgba(46,125,255,.35),inset 0 1px 0 rgba(255,255,255,.18)}
.nav.on::before{height:0}
.nav:hover{background:rgba(255,255,255,.06)}

/* 页面标题：左侧金色竖条 */
h2{position:relative;padding-left:14px;font-size:24px;letter-spacing:-.6px}
h2::before{content:"";position:absolute;left:0;top:.18em;bottom:.18em;width:3px;border-radius:2px;
  background:linear-gradient(180deg,var(--acc),var(--acc2))}
.lead{color:var(--sub)}

/* 统计卡：数字更大、首卡金色、描边更亮（光晕收细，只作暗示） */
.stats{gap:14px}
.stat{border:1px solid var(--line);background:var(--d2);border-radius:14px}
.stat .v{font-size:34px;font-weight:500}
.stat:first-child .v{color:#fff}
.stat::after{right:-10px;top:-10px;width:56px;height:56px;filter:blur(6px);
  background:radial-gradient(circle,rgba(34,211,238,.18),transparent 68%)}
.stat:hover{transform:translateY(-3px);border-color:rgba(46,125,255,.45)}

/* 卡片：hover 带主色描边，更强的层次 */
.mem{border-radius:12px}
.mem:hover{border-color:rgba(46,125,255,.40);box-shadow:0 8px 26px rgba(0,0,0,.42)}

/* 主按钮：金色描边 → hover 填充 */
/* 主按钮：蓝色实心（更大胆） */
.btn{background:rgba(46,125,255,.14);border-color:rgba(46,125,255,.42);color:#cfe0ff;font-weight:500}
.btn:hover{background:linear-gradient(135deg,var(--acc),#1f5fd6);color:#fff;
  border-color:transparent;box-shadow:0 6px 18px rgba(46,125,255,.38)}
.mini{border-color:var(--line2)}

/* 进度条改用主色 → 辅色 */
#topbar{background:linear-gradient(90deg,var(--acc),var(--acc2))}

/* 搜索框 / 输入框聚焦：金色环 */
.toolbar input:focus,.textin:focus{border-color:var(--acc);box-shadow:0 0 0 3px rgba(46,125,255,.20)}
...
/* 列表分组标题加金色小点 */
.listhead .t{display:inline-flex;align-items:center;gap:8px}
.listhead .t::before{content:"";width:6px;height:6px;border-radius:50%;background:var(--acc2);
...
.ngroup{margin:14px 0 6px}

/* 统计 → 仪表条 */
.stats{display:flex;gap:0;background:var(--d2);border:1px solid var(--line);
  border-radius:18px;padding:4px 0;overflow:hidden;margin-bottom:18px}
.stat{flex:1;background:transparent;border:none;border-right:1px solid var(--line);
  border-radius:0;padding:16px 20px;text-align:left}
.stat:last-child{border-right:none}
.stat .v{font-size:44px;line-height:1.05;font-weight:500}
.stat:hover{transform:none;background:rgba(46,125,255,.07)}
.stat::after{display:none}

/* 健康度仪表 */
.health{display:grid;grid-template-columns:auto auto minmax(260px,1fr);gap:14px 22px;
  align-items:center;background:var(--d2);
  border:1px solid var(--line);border-radius:16px;padding:18px 22px;margin-bottom:16px}
.health .hscore{display:flex;align-items:baseline;gap:6px}
.health .hscore b{font-size:46px;font-weight:500;line-height:1;
  font-variant-numeric:tabular-nums}
.health .hscore i{font-size:13px;color:var(--sub);font-style:normal}
.health .hgrade{font-size:12px;padding:3px 9px;border-radius:999px;
  border:1px solid var(--line2);color:var(--sub)}
.health .hbars{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px 18px}
.health .hrow{font-size:12px;color:var(--sub);display:flex;align-items:center;gap:10px}
.health .hrow span:first-child{width:88px;flex-shrink:0}
.health .hbar{flex:1;height:5px;border-radius:3px;background:var(--d3);overflow:hidden}
.health .hbar i{display:block;height:100%;border-radius:3px;
  background:linear-gradient(90deg,var(--acc),var(--acc2));transition:width .5s var(--ease)}
.health .hchips{grid-column:1/-1;display:flex;flex-wrap:wrap;gap:6px}
.health .hchip{font-size:11.5px;color:var(--sub);border:1px solid var(--line);
  border-radius:999px;padding:2px 9px}
.health .hchip.warn{color:var(--warn);border-color:rgba(255,214,10,.35)}
...
.split-side .dbody{font-size:13.5px;line-height:1.75;color:var(--ink);
  white-space:pre-wrap;word-break:break-word;margin-bottom:14px}
.split-side .dmeta{font-size:11.5px;color:var(--faint);line-height:1.9;
  border-top:1px solid var(--line);padding-top:10px;margin-bottom:12px;word-break:break-all}
.split-side .dacts{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px}
.mem.sel{border-color:var(--acc);box-shadow:0 0 0 1px var(--acc),0 8px 26px rgba(46,125,255,.22)}
.dempty{color:var(--faint);font-size:12.5px;line-height:1.9}
/* 记忆列表：可折叠分组（常驻 / 最近） */
.ghead{display:flex;align-items:center;gap:9px;cursor:pointer;user-select:none;
  font-size:13.5px;font-weight:500;margin:18px 0 9px;padding:5px 0;
  border-bottom:1px solid var(--line)}
.ghead:first-child{margin-top:2px}
.ghead:hover{color:var(--acc)}
.ghead .cv{font-size:11px;color:var(--sub);transition:transform .2s var(--ease)}
.ghead.collapsed .cv{transform:rotate(-90deg)}
.ghead .cnt{font-size:11.5px;color:var(--sub);border:1px solid var(--line);
...
.ghead::before{content:"";width:6px;height:6px;border-radius:50%;background:var(--acc);
  box-shadow:0 0 8px rgba(46,125,255,.6)}
.ghead.g-recent::before{background:var(--faint);box-shadow:none}
.gbody.hide{display:none}

/* 主从分工：左侧列表只显示摘要（6 行截断），完整内容在右侧详情 */
.split-main .mem .content{
  display:-webkit-box;-webkit-line-clamp:5;-webkit-box-orient:vertical;
  overflow:hidden;cursor:pointer
}
.split-main .mem .meta{margin-top:8px}
@media (max-width:1150px){
  .split{grid-template-columns:1fr}
  .split-side{position:static;min-height:auto;max-height:none;overflow:visible}
}
/* 窄窗口：统计条改 2×2 网格（grid 不会裁切内容） */
@media (max-width:1000px){
  .stats{display:grid;grid-template-columns:1fr 1fr;overflow:visible;padding:0}
  .stat{border-right:none;border-bottom:1px solid var(--line);padding:14px 18px;border-radius:0}
  ...
  <main class="main">
    ...
    <div class="content">
      ...
      <section id="v-mem">
        ...
        <div class="listhead">
          <span class="t" id="list-title">最近记忆</span>
          <span>
      ...
      <section id="v-session" hidden>
        ...
        <div class="panel">
          ...
          <textarea id="s-text" style="margin-top:8px"
            placeholder="把对话粘贴到这里：支持「我: / AI:」这类聊天文本，也支持 JSONL / JSON 数组（Claude Code、Codex 的会话文件内容可直接贴）"></textarea>
          <div class="row" style="margin-top:10px">
            ...
            <button class="btn" id="s-save" disabled onclick="sessionSave()">确认归档</button>
          </div>
          <p class="hint" id="s-info">先点「解析预览」确认说话人识别无误，再点「确认归档」。内容重复的会话会自动跳过。</p>
        </div>
        <div class="listhead">
          <span class="t">自动扫描本机对话</span>
          <span>
            <button class="btn" onclick="runAutoScan()">立即扫描</button>
          </span>
        </div>
        ...
        <div class="listhead">
          ...
          <input class="textin" id="s-q" style="max-width:280px" placeholder="在原话里检索…"
                 onkeydown="if(event.key==='Enter')sessionSearch()">
        </div>
        <div class="handoff-out" id="s-view"></div>
        <div id="s-list"></div>
      </section>
      ...
      <section id="v-audit" hidden>
        ...
        <div class="row" style="margin-bottom:14px">
          <button class="btn" onclick="runAudit()">开始质检</button>
      ...
      <section id="v-clean" hidden>
        ...
        <div class="panel" style="margin-bottom:16px">
          <div class="listhead" style="margin-bottom:8px">
            <span class="t">④ 备份与归档（防"全丢"）</span>
            <span>
              <button class="btn" onclick="pickArchiveDir()">选择存放位置…</button>
              <button class="btn" onclick="doSnapshot()">立即备份</button>
              <button class="btn" onclick="openArchiveDir()">打开归档目录</button>
            </span>
          </div>
          <p class="lead" id="arch-line">加载中…</p>
          <p class="hint" id="arch-hint"></p>
          <div class="row"><input type="text" class="textin" id="arch-dir" placeholder="归档目录（可放在 D 盘 / 移动硬盘；留空 = 不启用）">
            <button class="btn" onclick="saveArchiveDir()">保存</button></div>
          <p class="hint">快照是**复制**当前记忆库，<b>不会移动或删除任何记忆</b>。为避免占用系统盘，默认不启用，
        ...
        <p class="hint">这些是当初采集记忆的源文件（WorkBuddy 工作区日志）。删掉文件＝删掉对应记忆的来源依据。
        ...
        <p class="hint">勾选后点「删除选中记忆」＝硬删除（删前自动全量备份，可用「记忆包」导入还原）。</p>
        <div id="cm-list"></div>
        <div class="msg" id="cm-msg" style="display:none"></div>

        <div class="listhead"><span class="t">③ 删除备份记录</span></div>
        <div id="bk-list"></div>
      </section>
      ...
      <section id="v-collect" hidden>
        ...
        <div class="panel">
          <div class="row">
            <button class="btn" onclick="doScan()">扫描本机</button>
            ...
            <button class="btn" id="ck-go" style="display:none" onclick="doCollect()">入库选中</button>
          </div>
      ...
      <section id="v-agents" hidden>
        ...
        <div class="row" style="margin-bottom:14px">
          <button class="btn" onclick="registerAll()">全部接入</button>
          <button class="btn" onclick="verifyMcp()">验证 MCP 服务</button>
        </div>
        <div class="panel">
          <div class="row">
            <input type="text" class="textin" id="add-path" placeholder="装在其他位置的 Agent？粘 mcp.json 路径或它所在目录">
        ...
        <p class="hint">接入后需重启对应的 Agent 才会生效。</p>
      </section>
      ...
      <section id="v-pack" hidden>
        <div class="panel" style="margin-bottom:14px">
          <div class="row" style="gap:8px;align-items:center">
            ...
            <span><b>同时导出「会话层（对话原文）」</b></span>
          </div>
          <p class="hint">默认只导出记忆（结论）。勾上这一项，会把归档的原始对话一起打包 ——
        ...
        <div class="panel">
          <div class="row">
            ...
            <button class="btn" onclick="doExport()">导出记忆包</button>
          </div>
          <p class="hint">导出文件格式 hippocampus-pack v1，包含内容 / 类型 / 重要度 / 项目 / 时间。</p>
        </div>
        <div class="panel">
          <div class="row">
        ...
        <div class="panel">
          ...
          <p class="hint">把项目的经验类记忆导出成 SKILL.md，写进 ~/.workbuddy/skills，各 Agent 可直接读取。</p>
        </div>
      ...
      <section id="v-handoff" hidden>
        ...
        <div class="panel">
          <div class="row">
            ...
            <button class="btn" onclick="doHandoff()">生成交接卡</button>
...
function show(v){
  ...
  ["mem","session","audit","clean","collect","agents","pack","handoff"].forEach(x=>document.getElementById("v-"+x).hidden=(x!==v));
...
if(["mem","session","audit","clean","collect","agents","pack","handoff"].indexOf(_h)>=0){ show(_h); }
...
async function loadStats(){
  ...
  document.getElementById("stats").innerHTML=cards.map((c,i)=>
    `<div class="stat" style="--i:${i}"><div class="k">${c[1]}</div><div class="v" data-to="${c[0]}">0</div></div>`).join("");
...
function renderDetail(id){
  ...
  if(!r){
    el.innerHTML='<h3>记忆详情</h3><div class="dempty">点左侧任意一条记忆，这里会显示完整内容与操作按钮。</div>';
    return;
  }
  el.innerHTML='<h3>'+(TL[r.mtype]||r.mtype)+'　#'+r.id+'</h3>'+
    '<div class="dbody">'+esc(r.content)+'</div>'+
    '<div class="dmeta">项目：'+esc(r.project||'—')+
      '<br>来源：'+esc(r.agent||'—')+
      '<br>重要度：P'+r.importance+'　'+(r.pinned?'已常驻':'非常驻')+
    ...
    '</div>'+
    '<div class="dmeta">处理建议：重复或过期项去「质检」页批量处理；要连源文件一起清掉去「清理」页。</div>';
}
...
function cardHtml(r,score,idx){
  return `<div class="mem" id="memcard-${r.id}" style="--i:${idx||0}" onclick="selectMem(${r.id})">
    <div class="top">
      <span class="ttag"><i style="background:${TC[r.mtype]||"#8e8e93"}"></i>${TL[r.mtype]||r.mtype}</span>
      ${r.project?`<span class="proj">${esc(r.project)}</span>`:""}
      ${r.pinned?'<span class="pri">常驻</span>':""}
      <span class="pri">P${r.importance}</span>
      ${score!==undefined?`<span class="score">相关度 ${score.toFixed(3)}</span>`:""}
    </div>
    <div class="content">${esc(r.content)}</div>
    <div class="meta">
      <span>#${r.id} · ${esc(r.agent||"unknown")} · ${r.created_at}</span>
      <span><button class="del" onclick="event.stopPropagation();pinById(${r.id},${r.pinned?0:1})">${r.pinned?"取消常驻":"设为常驻"}</button>
  ...
  document.getElementById("sf-list").innerHTML = SFILES.length ? (
    ...
    SFILES.map(function(f,i){
      return '<div class="mem"><div class="top">'+
        ...
        '<span class="pri">关联 '+f.memories+' 条'+(f.exact?"（精确）":"（按工作区估算）")+'</span>'+
...
async function loadBackups(){
  ...
  document.getElementById("bk-list").innerHTML = rows.length ? rows.map(function(b){
    return '<div class="mem"><div class="top"><span class="ttag"><i style="background:#5DCAA5"></i>备份</span>'+
      ...
      '<span class="pri">'+fmtSize(b.size)+'</span><span class="score">'+esc(b.time)+'</span></div>'+
...
async function loadList(){
  searching=false;
  document.getElementById("list-title").textContent="最近记忆";
  const proj=document.getElementById("proj-filter").value;
...
async function doScan(){
  ...
  el.innerHTML=rows.map((it,i)=>`
    <div class="mem ${it.in_db?"indb":""}">
      <div class="top">
        <input type="checkbox" class="ck" data-p="${esc(it.path)}" ${it.in_db?"disabled":""} ${it.in_db?"":"checked"}>
        <span class="ttag"><i style="background:${it.kind==="日志"?"#0a84ff":"#bf5af2"}"></i>${it.kind}</span>
        <span class="proj">${esc(it.source)}</span>
        <span class="score">${esc(it.date)}</span>
        ${it.in_db?'<span class="badge-no" style="font-size:11px">已入库</span>':""}
      </div>
      <div class="content" style="font-size:13px;color:var(--sub)">${esc(it.preview)}…</div>
    </div>`).join("");
}
function checkAll(v){
...
async function runAutoScan(silent){
  if(!silent){scanmsg("正在扫描本机对话（ZCode / Claude Code / Codex）…");progress(true);}
...
/* 打开面板即自动扫一次（静默） */
window.addEventListener("load",function(){setTimeout(function(){runAutoScan(true)},1200)});
...
async function loadSessions(){
  ...
  document.getElementById("s-list").innerHTML = rows.length ? rows.map(function(s){
    return '<div class="mem"><div class="top">' +
      '<span class="ttag"><i style="background:#bf5af2"></i>'+(s.agent?esc(s.agent):"未知来源")+'</span>' +
      (s.project?('<span class="proj">'+esc(s.project)+'</span>'):"") +
      '<span class="pri">'+s.msg_count+' 轮</span>' +
      '<span class="score">'+esc(s.created_at)+'</span></div>' +
      '<div class="content">'+esc(s.title)+'</div>' +
      '<div class="meta"><span>#'+s.id+(s.source_path?(' · '+esc(s.source_path)):"")+'</span>' +
      '<span><button class="del" onclick="extractSession('+s.id+')">抽记忆</button> ' +
      '<button class="del" onclick="openSession('+s.id+')">查看原文</button> ' +
      '<button class="del" onclick="delSession('+s.id+')">删除</button></span></div></div>';
  }).join("") : '<div class="empty">还没有归档的会话</div>';
}
async function extractSession(sid){
  document.getElementById("s-view").style.display="block";
  document.getElementById("s-view").textContent="正在抽取候选记忆…";
  var rows=await api("/api/extract?sid="+sid);
  var el=document.getElementById("s-view");
  if(!rows.length){el.textContent="这段会话里没有抽到候选记忆（可能内容太短或与已有记忆重复）";return}
  el.textContent="";
  el.style.display="none";
  document.getElementById("s-list").innerHTML=
    '<div class="msg">从会话 #'+sid+' 抽出 '+rows.length+' 条候选记忆（我=你的话，优先）。勾选后入库。</div>'+
    '<div class="row" style="margin-bottom:10px"><button class="btn" onclick="importCands()">入库选中</button>'+
    '<button class="btn" onclick="loadSessions()">取消</button></div>'+
...
async function openSession(sid){
  ...
  var el=document.getElementById("s-view");
  el.style.display="block";
  el.textContent="#"+sid+" "+r.session.title+"\n"+
...
async function delSession(sid){
  if(!confirm("删除会话 #"+sid+" 及其全部原文？（不可恢复）"))return;
...
async function sessionSearch(){
  ...
  document.getElementById("s-list").innerHTML = rows.length ? rows.map(function(r){
    return '<div class="mem"><div class="top">' +
      ...
      '<span class="pri">第'+r.turn+'轮</span>' +
...
async function renderHealth(){
  ...
  var bars=Object.keys(h.penalties||{}).map(function(k){
    var v=h.penalties[k], pct=Math.round(v/25*100);
    return '<div class="hrow"><span>'+k+'</span>'+
      '<span class="hbar"><i style="width:'+pct+'%"></i></span>'+
      '<span style="width:44px;text-align:right">-'+v.toFixed(1)+'</span></div>';
  }).join("");
  var chips=Object.keys(h.counts||{}).map(function(k){
    var v=h.counts[k], bad=(k!=="重复组"&&k!=="疑似同义"&&k!=="可能矛盾"&&k!=="长期未更新")?v:(v>0?1:0);
    return '<span class="hchip'+(v>0?" warn":"")+'">'+k+" "+v+'</span>';
  }).join("");
  document.getElementById("health").innerHTML=
    '<div class="hscore"><b>'+h.score+'</b><i>/100</i></div>'+
    '<div><div class="hgrade">'+h.grade+" · "+h.grade_text+'</div></div>'+
...
h1{font-family:Georgia,serif;font-size:24px;margin-bottom:6px}
.warn{background:#3a3a3c;border:1px solid rgba(255,255,255,.12);border-radius:10px;
...
pre{background:#111;border:1px solid rgba(255,255,255,.08);border-radius:10px;padding:16px;
white-space:pre-wrap;font-family:Consolas,monospace;font-size:12.5px;line-height:1.75}
iframe{width:100%;height:460px;border:1px solid rgba(255,255,255,.12);border-radius:10px;margin-top:8px}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if u.path in ("/", "/index.html"):
            ...
            _fp = (body.get("path") or "").strip()
            if _fp and os.path.isdir(_fp):
                try:
                    if os.name == "nt":
                        os.startfile(_fp)          # noqa: 打开资源管理器
                    else:
                        import subprocess as _sp
                        _sp.Popen(["xdg-open", _fp] if os.name != "darwin" else ["open", _fp])
                    self._json({"ok": True})
                except Exception as e:
                    self._json({"error": str(e)})
            else:
                self._json({"error": "目录不存在：%s" % _fp})
            ...
            _ico = os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets", "icon.png")
