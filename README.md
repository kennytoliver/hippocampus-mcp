...
记忆层（结论）   「决策：技术选型定为静态博客，不上数据库」（短、可直接用）
```

Agent 开局需要的是**结论**（省 token、直接可用），而不是几十万字的聊天记录；
想知道"当时 AI 到底说了什么"时，再去会话层搜原文。
...
| 决策 | 选择 + 理由 | 技术选型定为静态博客 |
| 偏好 | 你的习惯 | 代码注释用中文 |
| 事实 | 客观信息 | 域名年费 80 元、明年到期 |
| 经验 | 做事方法 | 先构建再上传 |
| 踩坑 | 教训 | DNS 生效要等 10 分钟 |
| 背景 | 长篇上下文（多为采集自动入库） | 整段工作日志 |
| 摘要 | 长内容的压缩版 | 文档速览 |
...
- [x] 项目交接卡
- [x] 网页面板 + 记忆包 + Agent 体检
- [x] 一键接入（面板直接写各 Agent 的 MCP 配置）
- [x] 会话层（对话原文归档 + 原话召回 + 时间线回看）
- [x] 自动扫描本机对话（ZCode SQLite 已实测；Claude Code / Codex 同一解析器，未实机验证）
- [x] 记忆质检报告导出（Markdown）
- [x] Agent 接入覆盖国内外主流 15 款产品（含 ZCode 嵌套结构）
- [x] 记忆模块（常驻记忆 + **七查质检 + 0-100 健康度** + 会话抽记忆 + 记忆转技能 + 零依赖检索加权）
- [ ] 语义检索（需本地 embedding 模型，暂不做以保零依赖）
- [x] 历史会话批量导入 P1（WorkBuddy 日志 + skills，采集中心）
- [x] 历史会话批量导入 P2（Codex / Claude Code jsonl 解析已实现，**未实机验证**）
- [ ] 查询扩展与分词升级
